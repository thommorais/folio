...
const Clients = () => {
	...
	const domains = domainState.status === 'ready' ? domainState.domains : []

	return (
		<div className='mx-auto w-full max-w-5xl space-y-8'>
			<div className='flex w-full flex-col items-center pt-6 pb-4 text-center'>
				<WelcomeGreeting />
				{state.status === 'ready' && <SummaryTicker insights={buildInsights(state.clients, domains)} />}
			</div>
			...
			{state.status === 'ready' && state.clients.length === 0 && <p className='text-dim text-sm'>No clients yet.</p>}
